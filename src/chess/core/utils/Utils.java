...
public class Utils {
  ...
  public static String calculateDragMove(int mX, int mY, int mX1, int mY1, int squareS) {
    //This will determine the co-ordinates of the point where the user clicked
    return "" + (mY / squareS) + (mX / squareS) + (mY1 / squareS) + (mX1 / squareS);
  }
  ...
  public static String convertToAlgebraic(Move m) {
    ...
    if (m.getPieceC() != null) { //capture
      if (m.getPiece() == PieceType.PAWN) { //pawn is being moved
        r = "" + ((char) (97 + x1)) + "x" + ((char) (97 + x2)) + algY[y2]; //pawn capture to a location
      } else {
        r = "" + Type.tPieceString[m.getPiece().ordinal()] + "x" + ((char) (97 + x2)) + algY[y2]; //other piece capture
      }
    } else if (m.getType() == MoveType.PROMOTION) { //promotion
      r = "" + ((char) (97 + x2)) + algY[y2] + Type.tPieceString[m.getPiece().ordinal()];
    } else if (m.getType() == MoveType.CASTLE) { //castle, queen side or king side
      if (m.getLS() == 0) { r = "0-0-0"; } //Queen side castle
      else if (m.getLS() == 1) { r = "0-0"; } //King side castle
    } else if (m.getType() == MoveType.ENPASSANT) { //en passant, specific characters required
      r = "" + ((char) (97 + x1)) + "x" + ((char) (97 + x2)) + algY[y2] + "e.p.";
    } else { //normal move
      r = "" + Type.tPieceString[m.getPiece().ordinal()] + ((char) (97 + x2)) + algY[y2];
    }
