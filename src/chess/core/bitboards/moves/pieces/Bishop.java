...
public class Bishop extends Piece {
	private long osBoardManipulation(long pieceBB, long empty) {
		...
		if (pieceBB != 0) {
			while (pieceBB != 0) {
				...
				long diag = ((occ & diagMask) - bb1) ^ Long.reverse(Long.reverse(occ & diagMask) - bb2); //Gets possible slider moves in diagonal
				long aDiag = ((occ & aDiagMask) - bb1) ^ Long.reverse(Long.reverse(occ & aDiagMask) - bb2);	//Gets moves in anti-diagonal
	...
	public long wGetPossiblePieces(long pieceBB, long empty, long bOcc) {
		return wGetPossibleTargets(pieceBB, empty) | wGetPossibleCaptures(pieceBB, bOcc, empty); //Gets white moves and captures
	}
	@Override
	public long bGetPossiblePieces(long pieceBB, long empty, long wOcc) {
		return bGetPossibleTargets(pieceBB, empty) | bGetPossibleCaptures(pieceBB, wOcc, empty); //Gets black moves and captures
	}
}
